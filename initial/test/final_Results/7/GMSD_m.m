function [score, quality_map] = GMSD_m(Y1, Y2)
...
% This is modified. (This doesn't downsample the inputs)
...
if length(size(Y1))>2
    Y1 = double(rgb2gray(Y1));
    Y2 = double(rgb2gray(Y2));
else
    Y1 = double(Y1);
    Y2 = double(Y1);
end
...
quality_map = quality_map(2:end-1, 2:end-1);
