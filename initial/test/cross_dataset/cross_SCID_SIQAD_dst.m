...
for the_parameter = 1:14
    ...
    for dst_udx =1:6
        ...
        train_d_dst(:,:,dst_udx) = featrix_dst_SCID(dsts_SCID,1:feactor_length);
        train_l_dst(:,dst_udx) = featrix_dst_SCID(dsts_SCID,feactor_length+1);
        test_d_dst(:,:,dst_udx) = featrix_dst_SIQAD(dsts_SIQAD,1:feactor_length);
        test_l_dst(:,dst_udx) = featrix_dst_SIQAD(dsts_SIQAD,feactor_length+1);
    end
    ...
    for dst_udx = 1:6
        ...
        spear_results_dq_dst(1, dst_udx, the_parameter) = corr(predict_score, test_l_dst(:,dst_udx),'type','Spearman');
        [spear_results_dq_dst(2, dst_udx, the_parameter), spear_results_dq_dst(3, dst_udx, the_parameter)]=...
            PearsonLC(predict_score, test_l_dst(:,dst_udx));
        spear_results_dq_dst(4, dst_udx, the_parameter) = corr(predict_score, test_l_dst(:,dst_udx),'type','Kendall');
    end
    clear train_d_dst train_l_dst test_d_dst test_l_dst
end
save('cross_p_dst_SCID_SIQAD.mat', 'spear_results_dq_dst');