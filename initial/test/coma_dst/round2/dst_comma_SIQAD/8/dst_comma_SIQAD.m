...
for dst_idx = 1:7
    ...
    [featrix_rows, featrix_cols] = size(featrix_FR_SIQAD);
    feactor_length = featrix_cols-5;
    data = featrix_FR_SIQAD(:,1:feactor_length);
    label = featrix_FR_SIQAD(:,feactor_length+1);
    
    rows_of_distortion = find(featrix_FR_SIQAD(:,feactor_length+3)==dst_idx);
    %%
    cost_power = -3:2:15;
    gamma_power = -15:2:3;
    %%
    ...
    for aa = cost_set
        ...
        for bb = gamma_set
            ...
            for i = 1:N
                train = ismember(featrix_FR_SIQAD(:, feactor_length+2),C(i,:));
                test = ~train;
                %         rows_of_distortion = find(featrix_FR_SIQAD(:,feactor_length+3)==dst_idx);
